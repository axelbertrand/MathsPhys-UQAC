...
void processInputs(std::vector<InputsManager::Intention>& pendingIntentions, const opengl_wrapper::OpenGlWrapper& openglWrapper,
...
int main()
{
	...
	glfwSetWindowUserPointer(mainWindow, &inputsManager); //save the manager's pointer to the window to be able to access it in the inputs callback function
	...
	while (!openglWrapper.windowShouldClose(mainWindow))
	{
		...
		std::vector<InputsManager::Intention> pendingIntentions = inputsManager.getPendingIntentions();
		inputsManager.clearIntentions();

		// logic
		processInputs(pendingIntentions, openglWrapper, particles);
...
void processInputs(std::vector<InputsManager::Intention>& pendingIntentions, const opengl_wrapper::OpenGlWrapper& openglWrapper, 
	std::vector<Particle>& particles)
{
	while (!pendingIntentions.empty())
	{
		InputsManager::Intention intention = pendingIntentions.back();
		pendingIntentions.pop_back();
